...
class AccessibilityEventTaskManager(val context: Context) {
    ...
    private val ignorePackage: MutableList<String> =
        mutableListOf()   //Default ignore myself or trigger self issue
    ...
    init {
        ...
        val hasSetStr = SharedPreferenceUtil.getPref(
            context, Constants.PREF_TRIGGER_IGNORE_PACKAGE,
            Constants.MY_PACKAGE_NAME
        )!!

        val strArr = hasSetStr.split(",")
        ignorePackage.addAll(strArr)
    ...
    internal fun ignoreAdd(target: String){
        ignorePackage.add(target)
        saveIgnored()
    }

    internal fun ignoreRemove(target: String) {
        for ((index, i) in ignorePackage.withIndex()) {
            if (i == target) {
                ignorePackage.removeAt(index)
                saveIgnored()
                return
            }
        }
    ...
    internal fun ignoreContains(target: String): Boolean {
        for (i in ignorePackage) {
            if (i == target)
                return true
        }
        return false
    ...
    private fun ignoredPackToStr(): String {
        var out = ""
        for ((index, i) in ignorePackage.withIndex()) {
            out += i
            if (index != ignorePackage.size - 1) {
                out += ","
            }
        }
        return out
    ...
    private fun saveIgnored(){
        SharedPreferenceUtil.put(context,Constants.PREF_TRIGGER_IGNORE_PACKAGE,ignoredPackToStr())
    }
